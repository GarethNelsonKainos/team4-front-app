...
describe("API Client - uploadCV", () => {
	beforeEach(() => {
		vi.clearAllMocks();
	});

	describe("uploadCV function", () => {
		it("should export uploadCV function", () => {
			expect(typeof apiClient.uploadCV).toBe("function");
		});
		...
		it("should return object with success, data, error, and status properties", async () => {
			...
			expect(apiClient.uploadCV).toBeDefined();
		});

		it("should be an async function", async () => {
			const result = apiClient.uploadCV.constructor.name;
			expect(result).toBe("AsyncFunction");
		});
	});

	describe("uploadCV integration scenarios", () => {
		// Note: Detailed testing of uploadCV with various error scenarios
